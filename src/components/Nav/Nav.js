...
const Header = () => {
  ...
  return (
    <>
      ...
      > */}
      <header
        className={`top-0 left-0 z-[9999] w-full bg-white shadow-md dark:bg-transparent dark:bg-opacity-90 backdrop-blur-sm transition-all duration-300  ${
              ...
              <ul className="flex items-center space-x-6">
                {menuData.map((menuItem) => (
                  <li key={menuItem.id} className="group relative">
                    {!menuItem.path ? (
                      <>
                        <span className="flex items-center text-base font-medium text-dark dark:text-white cursor-pointer">
                          {menuItem.title}
                        ...
                        {/* Submenu (hover only) */}
                        <div className="invisible absolute left-2/2 top-full mt-6 z-50 w-[900px] -translate-x-1/2 rounded-md bg-white dark:bg-black border border-gray-800 p-6 opacity-0 shadow-lg transition-all duration-300 group-hover:visible group-hover:opacity-100 group-hover:top-full">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {Object.values(menuItem.submenu).map((group, i) => (
                              <div key={i}>
                                <h4 className="!text-xl mb-2 !text-transparent bg-clip-text bg-gradient-to-r from-[#17b3ac] via-blue-500 to-blue-300 mb-2 text-sm font-bold text-gray-700 dark:text-gray-300">
                                  {group.title}
                                </h4>
                                {group.items.map((subItem) => (
                                  <Link
                                    key={subItem.id}
                                    href={subItem.path}
                                    className={`block rounded py-1 hover:text-primary dark:hover:text-primary ${
                      ...
                      <Link
                        ...
                        className={`text-base font-medium ${
                          isActive(menuItem.path) ? 'text-primary' : 'text-dark dark:text-white hover:text-primary'
                        }`}
                      >
                        {menuItem.title}
                      </Link>
