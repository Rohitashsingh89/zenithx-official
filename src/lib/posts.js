...
export function servicePathBySlug(slug) {
  return `/service/${slug}`;
}
...
export async function getAllPostsWithoutServices(options = {}) {
  const { queryIncludes = 'index' } = options;
  ...
  let posts = data?.data.posts.edges.map(({ node = {} }) => node);
  posts = posts.filter((post) => {
    const categories = post?.categories?.edges?.map((edge) => edge.node) || [];
    return !categories.some((cat) => cat.slug === 'services');
  });
...
export async function getAllServicesPosts(options = {}) {
  const { queryIncludes = 'index' } = options;
...
export async function getRecentServicesPosts({ count, ...options }) {
  const { posts } = await getAllServicesPosts(options);
  const sorted = sortObjectsByDate(posts);
  return {
    posts: sorted.slice(0, count),
  };
...
export async function getRecentPostsWithoutServices({ count, ...options }) {
  const { posts } = await getAllPostsWithoutServices(options);
  const sorted = sortObjectsByDate(posts);
  return {
    posts: sorted.slice(0, count),
  };
...
export async function fetchRecentPosts(count = 5) {
  const { posts } = await getRecentPosts({ count, queryIncludes: 'index' });
  return posts;
}

export async function fetchRecentServicesPosts(count = 5) {
  const { posts } = await getRecentServicesPosts({ count, queryIncludes: 'index' });
  return posts;
}

export async function fetchRecentPostsWithoutServices(count = 5) {
  const { posts } = await getRecentPostsWithoutServices({ count, queryIncludes: 'index' });
  return posts;
}
...
export async function getAllPaginatedPostsWithoutServices({ currentPage = 1, ...options } = {}) {
  const { posts } = await getAllPostsWithoutServices(options);
  const postsPerPage = await getPostsPerPage();
  const pagesCount = await getPagesCount(posts, postsPerPage);
...
export async function getPaginatedAllServicesPosts({ currentPage = 1, ...options } = {}) {
  const { posts } = await getAllServicesPosts(options);
  const postsPerPage = await getPostsPerPage();
  const pagesCount = await getPagesCount(posts, postsPerPage);
...
export async function getPageBySlug(slug) {
  ...
  try {
    ...
    console.error(`Failed to fetch page by slug: ${slug}`, e);
    return {
      page: null,
    };
  }
